...
export class RegisterPageComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    ...
    this.registerForm = this.formBuilder.group({
      type_user: new FormControl(undefined, [Validators.required]),
      ...
      retype_password: new FormControl(undefined, [
        Validators.required,
        Validators.minLength(5),
        Validators.required,
        Validators.maxLength(255),
  ...
  onSubmit(): void {
    if (
      ...
      this.notification.error(
        this.translate.instant("ERROR_PASSWORD_NOT_MATCH")
      );
      return;
    }
    this.register(
      ...
      this.registerForm.value["password"],
      this.registerForm.value["type_user"],
  ...
  register(
    ...
    this.loading = true;
    this.userService
      .register(username, email, password, type_user, first_name, last_name)
      .subscribe(
        (result) => {
          this.loading = false;
          this.notification.successText(
            this.translate.instant("USER_REGISTERED")
          );
          this.navigate(["/login"]);
        },
        (err) => {
          ...
          if (err.status === 400) {
            this.notification.error(
              this.translate.instant("ERROR_USER_ALREADY_EXISTS")
            );
          } else {
